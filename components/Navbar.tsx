...
const heroWords = ["Grow,", "Learn,", "and", "Build", "Your", "Future"];
...
export default function Navbar({ showHero = true }: { showHero?: boolean }) {
  ...
  return (
    <header className="bg-white text-[#191919] dark:bg-[#191919] dark:text-white">
      ...
      {showHero ? (
        <section className="relative overflow-hidden bg-[#7427b3] px-6 pb-16 pt-18 text-center text-white md:px-10 md:pb-20 md:pt-22 lg:pb-24 lg:pt-24">
          ...
          <div className="relative mx-auto max-w-[1180px] animate-jmo-fade-up">
            ...
            <h1 className="hero-headline mx-auto mt-10 max-w-6xl text-4xl font-bold leading-[1.02] tracking-normal md:text-6xl lg:text-7xl">
              <span className="flex flex-wrap justify-center gap-x-5 gap-y-2">
                {heroWords.map((word, index) => (
                  <span
                    key={word}
                    className="animate-jmo-word-reveal inline-block"
                    style={{ "--word-delay": `${index * 110}ms` } as React.CSSProperties}
                  >
                    {word}
                  </span>
                ))}
            ...
            <p className="mx-auto mt-8 max-w-4xl text-lg leading-8 text-white/90 md:text-xl">
